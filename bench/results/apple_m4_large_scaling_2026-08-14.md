...
Raw benchmark record for mojoboost CPU, mojoboost Metal, and LightGBM CPU on
...
The mojoboost driver used three alternating CPU/GPU repeats for every seed;
...
- mojoboost GPU training: 4.289-4.382 s, mean 4.349 s
- mojoboost CPU training: 11.094-11.706 s, mean 11.382 s
- LightGBM 1-thread training: 8.903-8.985 s, mean 8.932 s
- LightGBM 10-thread training: 2.701-2.951 s, mean 2.818 s
- Metal is 2.62x faster than mojoboost CPU and 2.05x faster than LightGBM
  1-thread on the means
- Metal is 54% slower than LightGBM 10-thread on training and 44% slower on
  binning plus training
- observed process maximum RSS: mojoboost 0.84-0.96 GB; LightGBM about 1.66 GB

## 5,000,000 rows x 50 features

Seed 0 used three alternating mojoboost CPU/GPU repeats. Seeds 1 and 2 used
...
- mojoboost GPU training: 16.684-17.483 s, mean 17.162 s
- mojoboost CPU training: 55.138-57.918 s, mean 56.902 s
- LightGBM 1-thread training: 45.129-46.487 s, mean 45.626 s
- LightGBM 10-thread training: 12.217-12.916 s, mean 12.462 s
- Metal is 3.32x faster than mojoboost CPU and 2.66x faster than LightGBM
  1-thread on the means
- Metal is 38% slower than LightGBM 10-thread on training and 35% slower on
  binning plus training
- observed process maximum RSS: mojoboost 2.88-3.04 GB; LightGBM 6.02-6.50 GB
...
arrays and LightGBM's dataset in one process, while the mojoboost driver runs
...
On this workload, Metal scales better than mojoboost CPU as row count grows
