/* mojotrees C ABI.
 *
 * A small, stable C interface to mojotrees: train a model from a dense
 ...
 *    boundary. No mojotrees struct layout is exposed, so a mojotrees
 ...
 *    a struct layout.
 * 3. Every fallible function returns int32_t: MOJOTREES_OK (0) or a
 *    negative MOJOTREES_ERROR_* code. The human readable reason goes into
 ...
 *    mojotrees_*_free call. Those free functions accept NULL.
 * 6. Pointer arguments are checked. A NULL where a value is required is
 *    MOJOTREES_ERROR_INVALID_ARGUMENT, never a crash.
 *
 * Thread safety: no global state is involved, so calls on distinct handles
 * may run concurrently. A single MojoTreesError, or a single model being
 ...
 *
 * Nothing here reimplements mojotrees. Every call forwards to the same Mojo
 ...
 *
 * Build: capi/build.sh, which produces capi/libmojotrees.{dylib,so}.
 ...
 *
 * Copyright the mojotrees authors. Apache-2.0.
 */
 ...
 * Check it with mojotrees_abi_version() when loading the library
 * dynamically.
 *
 ...
 *   2  mojotrees_predict_ex (iteration ranges and device selection),
 *      mojotrees_model_num_iterations, mojotrees_gpu_available,
 *      mojotrees_model_dump_json, mojotrees_string_free.
 *   3  mojotrees_feature_importance, mojotrees_parameter_keys.
 */
#define MOJOTREES_ABI_VERSION 3

/* Status codes. Success is 0; every failure is negative. */
#define MOJOTREES_OK 0
#define MOJOTREES_ERROR_INVALID_ARGUMENT (-1)
#define MOJOTREES_ERROR_TRAINING (-2)
#define MOJOTREES_ERROR_IO (-3)
#define MOJOTREES_ERROR_UNSUPPORTED (-4)
 ...
 * MOJOTREES_ERROR_UNSUPPORTED, carrying the reason, when no accelerator is
...
#define MOJOTREES_DEVICE_CPU 0
#define MOJOTREES_DEVICE_GPU 1
#define MOJOTREES_DEVICE_AUTO 2

/* Flags for mojotrees_predict_ex. Undefined bits are rejected rather than
 * ignored, so a flag a newer mojotrees understands cannot be silently
 * dropped by an older one. Since ABI version 2. */
#define MOJOTREES_PREDICT_RESPONSE 0
#define MOJOTREES_PREDICT_RAW 1

/* What mojotrees_feature_importance measures. SPLIT counts how many splits
 ...
 * version 3. */
#define MOJOTREES_IMPORTANCE_SPLIT 0
#define MOJOTREES_IMPORTANCE_GAIN 1

/* A trained or loaded model. Opaque: the layout is a mojotrees internal. */
typedef struct MojoTreesModel MojoTreesModel;

/* Receives the message for the most recent failed call it was passed to. */
typedef struct MojoTreesError MojoTreesError;

/* ---------------------------------------------------------------- version */

/* The ABI version this library was built with. */
int32_t mojotrees_abi_version(void);

/* The library version. Any of the three out pointers may be NULL. */
void mojotrees_library_version(int32_t *major, int32_t *minor,
                               int32_t *patch);

/* 1 when MOJOTREES_DEVICE_GPU can be honored by this build on this machine,
 ...
 * and an uncovered one is MOJOTREES_ERROR_UNSUPPORTED with the reason in
 * the error object. Since ABI version 2. */
int32_t mojotrees_gpu_available(void);

/* ------------------------------------------------------------------ error */

/* A new, empty error object. Returns NULL only if allocation fails. */
MojoTreesError *mojotrees_error_create(void);

/* Free an error object. NULL is accepted and does nothing. */
void mojotrees_error_free(MojoTreesError *error);
...
const char *mojotrees_error_message(const MojoTreesError *error);
 ...
 * call returns. Free the model with mojotrees_model_free. */
int32_t mojotrees_train_dense(const double *data, int64_t n_rows,
                              int64_t n_features, const double *labels,
                              const double *weights, const char *parameters,
                              MojoTreesModel **out_model,
 ...
 *
 * n_features must equal mojotrees_model_num_features(model). Values are
 * written row-major as out_values[r * k + c] where k is
 * mojotrees_model_num_classes(model), so out_len must be at least
 * n_rows * k. Nothing is written unless the whole call succeeds. */
int32_t mojotrees_predict(const MojoTreesModel *model, const double *data,
 ...
 * and validation are exactly as for mojotrees_predict. */
int32_t mojotrees_predict_raw(const MojoTreesModel *model,
                              const double *data, int64_t n_rows,
                              int64_t n_features, double *out_values,
                              int64_t out_len, MojoTreesError *error);
 ...
 *   flags            MOJOTREES_PREDICT_RESPONSE or MOJOTREES_PREDICT_RAW.
 *                    Any other value is MOJOTREES_ERROR_INVALID_ARGUMENT.
 *   device           a MOJOTREES_DEVICE_* value.
 *
...
int32_t mojotrees_predict_ex(const MojoTreesModel *model, const double *data,
                             int64_t n_rows, int64_t n_features,
...
/* Write the model to `path` in the versioned mojotrees text format. */
int32_t mojotrees_save_model(const MojoTreesModel *model, const char *path,
                             MojoTreesError *error);

/* Load a model written by mojotrees_save_model, or by the Mojo or Python
...
int32_t mojotrees_load_model(const char *path, MojoTreesModel **out_model,
                             MojoTreesError *error);

/* ------------------------------------------------------------- accessors */

/* Number of features the model expects in a prediction matrix. */
int32_t mojotrees_model_num_features(const MojoTreesModel *model,
                                     int64_t *out_value,
                                     MojoTreesError *error);

/* Number of values the model predicts per row: the class count for a
 * multiclass model, 1 for every single-output model. */
int32_t mojotrees_model_num_classes(const MojoTreesModel *model,
                                    int64_t *out_value,
                                    MojoTreesError *error);
...
int32_t mojotrees_model_num_trees(const MojoTreesModel *model,
                                  int64_t *out_value,
                                  MojoTreesError *error);
 ...
 * the tree count, is the unit mojotrees_predict_ex slices in. Since ABI
 * version 2. */
int32_t mojotrees_model_num_iterations(const MojoTreesModel *model,
                                       int64_t *out_value,
                                       MojoTreesError *error);

/* --------------------------------------------------------------- inspect */

/* The model as JSON in the mojotrees inspection schema: the ensemble's
 ...
 * must release with mojotrees_string_free. On failure *out_text is
...
int32_t mojotrees_model_dump_json(const MojoTreesModel *model,
                                  char **out_text, MojoTreesError *error);

/* Per-feature importance, one double per feature in feature order.
 *
 *   importance_type  MOJOTREES_IMPORTANCE_SPLIT or MOJOTREES_IMPORTANCE_GAIN.
 *                    Any other value is MOJOTREES_ERROR_INVALID_ARGUMENT.
 *   out_values       receives mojotrees_model_num_features(model) doubles.
 ...
 * rather than a measurement; mojotrees_model_dump_json reports
 * has_split_gain, which tells the two apart. Nothing is written unless the
 * whole call succeeds. Since ABI version 3. */
int32_t mojotrees_feature_importance(const MojoTreesModel *model,
 ...
 * MOJOTREES_ERROR_UNSUPPORTED when a caller tries one.
 *
 * On success *out_text is a NUL terminated UTF-8 string the caller owns and
 * must release with mojotrees_string_free. On failure *out_text is
 * untouched. Since ABI version 3. */
int32_t mojotrees_parameter_keys(char **out_text, MojoTreesError *error);

/* --------------------------------------------------------------- destroy */

/* Free a string the library allocated, such as one from
 * mojotrees_model_dump_json. NULL is accepted and does nothing. Do not pass
 * it a mojotrees_error_message pointer: that one is owned by the error
 * object. Do not release it with free() either, since the library may not
 * share the caller's allocator. Since ABI version 2. */
void mojotrees_string_free(char *text);
 ...
 * for free(). */
void mojotrees_model_free(MojoTreesModel *model);
