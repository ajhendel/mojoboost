...
#' `mb.Dataset` is a plain R object. mojotrees bins inside training, so there
...
mb_as_matrix <- function(data, what) {
  if (inherits(data, c("dgCMatrix", "dgRMatrix", "dgTMatrix", "sparseMatrix"))) {
    stop(sprintf(
      "%s is a sparse matrix. mojotrees has no sparse training path yet; ",
      what
...
mb_as_label <- function(label, n) {
  if (is.factor(label)) {
    stop(
      "label is a factor. mojotrees trains on numeric labels: use ",
