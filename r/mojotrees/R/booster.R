...
#
# A trained model is an external pointer into the mojotrees engine plus the R
...
#' Predict with a mojotrees model
#'
...
#'   integer vector.
#'
#'   Class indices are `0`-based codes, not the original labels: mojotrees's
#'   model format does not store a label mapping, so translating them back is
#'   the caller's job.
#' @examples
#' X <- matrix(rnorm(200), ncol = 2)
#' y <- as.numeric(X[, 1] > 0)
#' model <- mojotrees(X, y, objective = "binary", nrounds = 20L)
...
predict.mb.Booster <- function(object, newdata,
  ...
  if (length(type) == 1L && type %in% c("leaf", "contrib")) {
    stop(sprintf(
      "mojotrees cannot predict %s yet",
...
#'   mojotrees does not record per-split hessian sums; and split gains are not
#'   part of the model file, so a model from [mb.load()] reports zero `Gain`
#'   and warns.
#' @examples
#' X <- matrix(rnorm(300), ncol = 3)
#' y <- 3 * X[, 1] - 2 * X[, 2] + rnorm(100, sd = 0.1)
#' model <- mojotrees(X, y, nrounds = 20L)
#' mb.importance(model)
#' @export
