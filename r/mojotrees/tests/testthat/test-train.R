...
test_that("mojotrees() is mb.train() without the Dataset step", {
  d <- make_regression()
  a <- mb.train(
  ...
  b <- mojotrees(d$X, d$y, params = list(objective = "regression"),
                 nrounds = 10L)
...
test_that("the objective shorthand and params agree, and cannot be doubled", {
  d <- make_binary()
  a <- mojotrees(d$X, d$y, objective = "binary", nrounds = 10L)
  b <- mojotrees(d$X, d$y, params = list(objective = "binary"), nrounds = 10L)
  expect_identical(predict(a, d$X), predict(b, d$X))
  expect_error(
    mojotrees(d$X, d$y, objective = "binary",
...
test_that("every round-count alias is one the engine really accepts", {
  ...
  # LightGBM's spelling, which mojotrees does not accept. It must fail as an
...
test_that("every documented objective trains", {
  ...
  for (objective in c("regression", "l2", "mse", "huber", "quantile",
                      "regression_l1", "mae")) {
    model <- mojotrees(d$X, d$y, objective = objective, nrounds = 5L)
    expect_equal(mb.num.iteration(model), 5L, info = objective)
  }
  counts <- make_multiclass()
  poisson <- mojotrees(counts$X, counts$y, objective = "poisson", nrounds = 5L)
