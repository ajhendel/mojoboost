...
test_that("what the file does not hold is reported honestly", {
  d <- make_regression()
  model <- mojoboost(d$X, d$y, nrounds = 15L)
  path <- tempfile(fileext = ".mbst")
  on.exit(unlink(path), add = TRUE)
  mb.save(model, path)
  reloaded <- mb.load(path)
...
test_that("saving creates missing parent directories", {
  ...
  # when the directory does not exist. mojoboost's writer creates the path.
  d <- make_regression()
  model <- mojoboost(d$X, d$y, nrounds = 5L)
...
test_that("a file that is not a model is rejected, not misread", {
  path <- tempfile(fileext = ".mbst")
  on.exit(unlink(path), add = TRUE)
  writeLines(c("this is not a mojoboost model"), path)
