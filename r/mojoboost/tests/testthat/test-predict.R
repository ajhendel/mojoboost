test_that("regression predictions are a plain vector and raw equals response", {
  d <- make_regression()
  model <- mojoboost(d$X, d$y, nrounds = 20L)
...
test_that("binary predictions are probabilities and raw is their logit", {
  d <- make_binary()
  model <- mojoboost(d$X, d$y, objective = "binary", nrounds = 30L)
  # LightGBM reports 1 class for binary; so does mojoboost.
...
test_that("multiclass predictions are a row-normalized matrix", {
  d <- make_multiclass()
  model <- mojoboost(d$X, d$y, objective = "multiclass",
...
test_that("NA and NaN are missing values, infinities are rejected", {
  ...
  model <- mojoboost(X, d$y, nrounds = 10L)
  pred <- predict(model, X)
  expect_false(any(is.na(pred)))

  X[1, 1] <- Inf
  expect_error(mojoboost(X, d$y, nrounds = 5L), "infinite")
