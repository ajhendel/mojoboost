#' Parameter keys mojoboost accepts
#'
...
#' The mojoboost engine version
#'
#' @return The version of the linked mojoboost C ABI, as a string.
#' @export
mb.version <- function() {
  .Call(R_mb_version)
}
...
#'   `params` to ask for it; mojoboost raises rather than falling back
...
mb_format_value <- function(name, value) {
  ...
  if (is.logical(value)) {
    # mojoboost has no boolean parameters today; a logical is almost always a
...
#'
#' mojoboost's C ABI takes LightGBM's `"key=value key=value"` form. This is
...
#
#   verbosity, verbose  mojoboost does not log through a parameter.
#   num_threads         thread count comes from MOJOBOOST_NUM_WORKERS.
...
# `num_trees` is deliberately absent: mojoboost does not accept it.
