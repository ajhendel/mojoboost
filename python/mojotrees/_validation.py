...
---------------------------
mojotrees's numeric rules live in ``src/mojotrees/validation.mojo``, which
...
through the estimators or through ``mojotrees.Dataset``.
...
# Mirrors of the ceilings in src/mojotrees/validation.mojo. They are checked
...
def check_shape(n_rows, n_features, name="X"):
    ...
    if n_rows > MAX_ALLOC_ELEMS // n_features:
        raise ValueError(
            ...
            f"{MAX_ALLOC_ELEMS} elements, which mojotrees will not allocate"
        )
...
def check_feature_names_match(fitted, incoming, name="X"):
    ...
    if set(fitted) == set(incoming):
        raise ValueError(
            ...
            "mojotrees matches features by position, so reorder the columns "
            f"to {list(fitted)}"
        )
...
def domain_checks_are_native():
    ...
    ``src/mojotrees/validation.mojo`` is where it went. See
