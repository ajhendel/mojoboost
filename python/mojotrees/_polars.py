...
it was handed, it refuses what polars can express and mojotrees cannot
...
zero copy for mojotrees: `_arrow` then materializes every column into a
...
_REFUSED_DTYPES = {
    ...
    "128-bit integer has no exact float64 reading and mojotrees trains on "
    "float64",
...
def preflight(frame, name="X"):
    """Refuse the columns polars can express and mojotrees cannot train on,
    ...
    for index, (column_name, dtype) in enumerate(_schema_pairs(frame)):
        ...
        raise ValueError(
            ...
            f"{dtype}, which mojotrees does not read as a feature: {advice}"
        )
