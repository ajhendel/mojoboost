...
cannot be represented -- is in `src/mojotrees/lgbm_model_io.mojo`. Nothing
...
already owns, which is exactly the drift `mojotrees.inspection` is already
...
**LightGBM's format is not mojotrees's.** `Booster.save_model` and
`Booster.model_to_string` speak mojotrees's own versioned format, which
...
to. Both conversions here go *through* a mojotrees model file for that
...
a Python conversion is the thing this module exists not to be.
"""
...
__all__ = [
    ...
    "interop_status",
    "unsupported_reason",
    "convert_to_mojotrees",
    "convert_from_mojotrees",
    "load_lightgbm_model",
    "save_lightgbm_model",
]
...
#: `src/mojotrees/lgbm_model_io.mojo`. Kept as data so the error message a
...
def _require(name):
    ...
    if hook is None:
        raise LightGBMInteropUnavailable(
            f"this mojotrees build does not expose {name!r}, so LightGBM "
...
def _warn_once():
    ...
    try:
        ...
        return
    _warnings.warn(
        f"mojotrees LightGBM model interop is experimental. {status}",
        UserWarning,
        stacklevel=3,
    )
...
def convert_to_mojotrees(lightgbm_path, model_path):
    ...
    mojotrees's own format and is what `Booster(model_file=...)` reads.
    """
    _warn_once()
    return _report(
...
def convert_from_mojotrees(model_path, lightgbm_path):
...
def load_lightgbm_model(lightgbm_path):
    """A `mojotrees.Booster` holding the model in a LightGBM model file.

    The conversion lands in a mojotrees model file first and the booster is
    ...
    with _tempfile.TemporaryDirectory() as tmp:
        ...
        convert_to_mojotrees(lightgbm_path, native)
        return Booster(model_file=native)


def save_lightgbm_model(booster, lightgbm_path):
    """Write a fitted `mojotrees.Booster` as a LightGBM model file.
    ...
    with _tempfile.TemporaryDirectory() as tmp:
        ...
        return convert_from_mojotrees(native, lightgbm_path)
