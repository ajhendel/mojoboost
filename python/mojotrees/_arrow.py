...
`mojotrees.dask.is_dask_collection()` does, so this module is import safe on
an install with no pyarrow and it costs nothing on `import mojotrees`.
...
mojotrees trains on it in place. It does not, and this module never says it
...
2. **The mojotrees side is not ready for it in any case.** The binding
   entry point (`dataset_create` in `bindings/_mojotrees.mojo`) takes one
...
its docstring. It never means "mojotrees read it in place".
...
A null becomes NaN, which is mojotrees's missing marker end to end (see
`src/mojotrees/binning.mojo`, which reserves a bin for it, and the
...
#: `mojotrees._Base._CATEGORY_LIMIT`. A dictionary with more entries than
...
def is_arrow_dataset(obj):
    ...
    core's (`src/mojotrees/sequence.mojo`, task 07); neither is something
...
def is_arrow_sparse_tensor(obj):
    ...
    turns the two that mojotrees's sparse path can use into what that path
    takes.
    """
    return (
...
def _refusal(arrow_type, column_name, index, matrix_name):
    ...
    return ValueError(
        ...
        f"{arrow_type}, which mojotrees does not read as a feature: {advice}"
    )
...
def _check_wide_int_chunk(values, column_name, index, matrix_name):
    ...
    check; mojotrees refuses, on the same principle that refuses
    ...
    raise ValueError(
        ...
        "mojotrees converts every feature to float64, so cast the column "
...
class BufferPlan:
    ...
    A description, and only a description. Nothing in mojotrees reads an
    ...
    - `zero_copy_eligible`: whether the *Arrow* side could be read in
      place, which is a strictly weaker statement than "mojotrees reads it
...
def sparse_to_scipy(obj, name="X"):
    ...
    arrays mojotrees's sparse path carries, and both expose `to_scipy()`,
    ...
    if "CSR" not in kind and "CSC" not in kind:
        raise TypeError(
            f"{name} is a {kind}, which mojotrees's sparse path does not "
...
def refuse_lazy(obj, name="X"):
    ...
    memory, and it hides the fact that mojotrees has a real streaming
    story in progress (`_sequence.py` on this side,
    `src/mojotrees/sequence.mojo` on the other).
    """
    raise TypeError(
        ...
        "mojotrees's batched input path"
    )
