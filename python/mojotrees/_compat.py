...
`python/mojotrees/` that any supported interpreter lacks, so there is nothing
...
**Where it is called from.** `python/mojotrees/__init__.py` calls
...
have: importing any submodule of `mojotrees` runs the package's `__init__`
first, so a check there runs before any other module can name the extension.
The modules that later say `from . import _mojotrees` are reading a module
...
#: artifact to measure.
_NO_GIL_REASON = (
    "the pinned MAX toolchain depends on python-gil, so no mojotrees "
...
def unsupported_interpreter():
    ...
    if not gil_enabled():
        return (
            "mojotrees does not support the free-threaded build of CPython "
            "%d.%d: %s"
            % (sys.version_info[0], sys.version_info[1], _NO_GIL_REASON)
        )
    if sys.version_info[:2] < EXTENSION_FLOOR:
        return (
            "mojotrees needs CPython %d.%d or newer and this is %d.%d. The "
...
def import_extension():
    """Import and return `mojotrees._mojotrees`.
