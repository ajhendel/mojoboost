...
`MOJOBOOST_DISABLE_GPU=1` is masking.
...
#: required to import mojoboost or to fit a model.
...
def _distribution_version():
    """The version recorded for the installed `mojoboost` distribution, or
    ...
    try:
        ...
        return metadata.version("mojoboost")
    except Exception:
        return None
...
def build_info():
    ...
    it" from "this build has one and `MOJOBOOST_DISABLE_GPU=1` is masking
    ...
    when a device is opened. See `mojoboost.device_selection` for the
    ...
    env = {
        ...
        if value != ""
        and (name.startswith("MOJOBOOST_") or name.startswith("MODULAR_"))
    }

    available = bool(_mojoboost.gpu_available())
    if available:
        gpu_path = "yes"
    elif env.get("MOJOBOOST_DISABLE_GPU") == "1":
        gpu_path = "unknown"
    else:
        gpu_path = "no"

    info = {
        "mojoboost": __version__,
...
def show_versions(file=None):
    """Print what this installation is, in the form a bug report needs.
        ...
        >>> import mojoboost
        >>> mojoboost.show_versions()          # doctest: +SKIP
    ...
    print(f"mojoboost {info['mojoboost']}", file=out)
    ...
    if info["env"]:
        ...
        line("(none set)", "MOJOBOOST_* and MODULAR_* are unset")
    ...
    if info["gpu_path_compiled_in"] == "no":
        ...
        notes.append(
            "MOJOBOOST_DISABLE_GPU=1 is masking whether this build has a\n"
    ...
    if distribution is not None and distribution != info["mojoboost"]:
        notes.append(
            f"The installed distribution says {distribution} and the "
            f"package says {info['mojoboost']}.\nOne is shadowing the "
