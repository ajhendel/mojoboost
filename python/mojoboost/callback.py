"""Training callbacks for the Python API.
    ...
    from mojoboost.callback import log_evaluation, record_evaluation
...
- `early_stopping()` configures the trainer's own early stopping rather than
  ...
  src/mojoboost/custom_metric.mojo. The one behavioral difference is which
  round survives: mojoboost truncates to the best round of the primary
...
#: `_write_reset`/`_read_reset` in bindings/_mojoboost.mojo: the two sides
...
_RESET_ALIASES = {
    ...
    # mojoboost's own spelling of the same parameter on the estimator.
...
# Phases and control codes; these mirror src/mojoboost/callback.mojo.
BEFORE_ITERATION = 0
AFTER_ITERATION = 1
...
def reset_parameter(**kwargs):
    ...
    raises rather than being ignored. See src/mojoboost/callback.mojo for why
