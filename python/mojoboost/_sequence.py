...
mojoboost one matrix. `Batches` and `materialize` take the pieces, check
...
bins fixed before any row is written. That core is `src/mojoboost/
sequence.mojo` and `src/mojoboost/external_memory.mojo` (task 07), and the
...
def _refuse_arrow_sparse(X, name="X", encoders=None):
    raise TypeError(
        f"{name} is an Arrow sparse tensor. mojoboost's sparse path takes "
...
def describe_input(data):
    ...
    Diagnostics: what `mojoboost.show_versions()` and a bug report want,
...
class BatchedInput:
    ...
    def dataset_kwargs(self, **extra):
        """Keyword arguments for `mojoboost.Dataset`.

            data = mojoboost._sequence.materialize(batches, label_column="y")
