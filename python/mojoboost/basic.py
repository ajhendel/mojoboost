...
The scikit-learn estimators in this package are one way to reach mojoboost.
...
- **`free_raw_data` defaults to False.** LightGBM frees the raw matrix on
  construction. mojoboost evaluates a dataset by predicting through the
...
- **`reference` is checked, not required.** A validation Dataset in
  ...
  mojoboost predicts validation rows through the model's own mapper, so a
...
def _objectives():
    ...
    from . import MojoBoostRegressor

    return MojoBoostRegressor._OBJECTIVES
...
def _task_of(objective):
    ...
    if callable(objective):
        raise ValueError(
            ...
            "callback trains through MojoBoostRegressor(objective=f)"
        )
    name = str(objective)
    ...
    if name in ("lambdarank", "rank_xendcg", "rank_xendcg_norm"):
        if name != "lambdarank":
            raise ValueError(
                ...
                "ranking objective mojoboost trains"
            )
...
class _Config:
    ...
    def __init__(self, params, num_boost_round=None):
        ...
        # MojoBoostRegressor._objective_param.
...
class Dataset:
    ...
    def get_field(self, field_name):
        """LightGBM's `get_field`, for the fields mojoboost keeps."""
        getters = {
        ...
        if field_name not in getters:
            raise ValueError(
                f"unknown field {field_name!r}; a mojoboost Dataset holds "
                + ", ".join(sorted(getters))
            )
...
class Booster:
    ...
    starts with no iterations:
        ...
        booster = mojoboost.Booster({"objective": "regression"}, train_set)
    ...
    def _build(self, rounds):
        ...
        if self._task == _eval.MULTICLASS:
            self._handle = _mojoboost.train_dataset_multiclass(handle, params)
        elif self._task == _eval.RANKING:
            self._handle = _mojoboost.train_dataset_ranker(handle, params)
        else:
            self._handle = _mojoboost.train_dataset(handle, params)
    ...
    def update(self, num_iteration=1):
        ...
        grow = (
            _mojoboost.booster_update_multiclass
            if self._n_classes
            else _mojoboost.booster_update
        )
        added = int(
    ...
    def _require_trainable(self):
        if self._train_set is None or self._config is None:
            raise ValueError(
                ...
                "carry. Train through mojoboost.train() to keep it"
            )
    ...
    def current_iteration(self):
        ...
        if self._n_classes:
            return int(_mojoboost.num_iterations_multiclass(self._handle))
        return int(_mojoboost.num_iterations(self._handle))
    ...
    def num_feature(self):
        ...
        if self._n_classes:
            return int(_mojoboost.n_features_multiclass(self._handle))
        return int(_mojoboost.n_features(self._handle))
    ...
    def add_valid(self, data, name):
        ...
        `add_valid`."""
        if not isinstance(data, Dataset):
            raise TypeError("a validation set must be a mojoboost.Dataset")
    ...
    def eval(self, data, name, metric=None):
        ...
        this objective. The value comes from src/mojoboost/metrics.mojo, the
        ...
        if not isinstance(data, Dataset):
            raise TypeError("eval() takes a mojoboost.Dataset")
        ...
        params = {
            ...
            # (see eval_metric in bindings/_mojoboost.mojo). A Booster read
        ...
        value = float(_mojoboost.eval_metric(code, params))
    ...
    def _from_estimator(cls, handle, estimator):
        ...
        booster._n_classes = (
            int(_mojoboost.n_classes(handle)) if multiclass else 0
        )
    ...
    def _copy_handle(self):
        ...
        if self._n_classes:
            return _mojoboost.copy_multiclass_model(self._handle)
        return _mojoboost.copy_model(self._handle)
...
def _continue(config, train_set, init_model):
    ...
    if not isinstance(init_model, Booster):
        raise TypeError(
            "init_model must be a mojoboost.Booster; read a saved model with "
