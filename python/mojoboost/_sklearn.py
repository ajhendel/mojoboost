...
not been run, so mojoboost does not claim to be a compliant estimator.
...
- `NotFittedError` subclasses scikit-learn's when it can, so `except
  ...
  `RuntimeError`, which is what mojoboost raised before, so existing
...
class NotFittedError(_SklearnNotFittedError, RuntimeError):
    ...
    Also a `RuntimeError`, which is what mojoboost raised before it grew a
    named exception.
    """


def estimator_tags(kind):
    """scikit-learn `Tags` for a mojoboost estimator, `kind` being
    ...
    proxy = type("_MojoBoostTagProxy", (mixin, BaseEstimator), {})()
    tags = proxy.__sklearn_tags__()
    # NaN is the missing-value marker (src/mojoboost/binning.mojo); sparse
...
class ParamsMixin:
    ...
    of the estimator's own `__init__`, and mojoboost's subclasses forward
    the shared ones through `**kwargs` instead. `get_params`, `set_params`,
    `clone`, `Pipeline`, and `GridSearchCV` all work either way, but
    `inspect.signature(MojoBoostRegressor)` does not list the inherited
    parameters, and this is one of the reasons mojoboost does not claim to
    ...
    def get_params(self, deep=True):
        ...
        parameter value that is itself an estimator; mojoboost has none
