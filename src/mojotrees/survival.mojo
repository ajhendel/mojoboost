...
default moves. `docs/design/CATBOOST_CATALOG.md` A26 and A27 carry the source
...
# 16 and 17, not 13 and 14. `catboost_ranking.mojo` took 13/14/15 for
# QueryRMSE, PairLogit and YetiRank in the same round. An objective code
# is a number in a serialized model, so two objectives sharing one would
# not be a merge conflict, it would be a model that loads as the wrong
# loss. Renumbered here on merge rather than there because ranking's
# codes were written first.
comptime COX = 16
...
def cox_grad_hess_into[
    ...
    docstring and catalog A26 for why that matters on ties.
