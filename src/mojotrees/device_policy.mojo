...
# 5: RESERVED for the concurrent refusal lane, which is adding three blocks
# (`enable_bundle`, `linear_tree`, and forced splits) on its own branch and
# took the number first. This lane skipped it rather than defining a second,
# incompatible meaning for "version 5", which is the one thing this counter
# exists to prevent. If that lane lands after this one, its note belongs
# above this line and nothing else moves.
