...
- **Position bias is a score offset.** `state.adjust_scores` produces
  ...
  through mojotrees's one implementation of it, and adds a per-round Newton
...
struct LabelGain(Copyable, Movable):
    ...
    LightGBM's default and mojotrees's only shipped one: `2^l - 1` for
...
def _pair_stream(seed: Int, iteration: Int, query: Int) -> UInt64:
    ...
    Counter based, like every other draw in mojotrees: the decision for pair
...
def query_folds(
    ...
    `python/mojotrees/cv.py::_chunk_folds` does, so the Mojo and Python
