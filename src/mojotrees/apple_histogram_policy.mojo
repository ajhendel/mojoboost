...
- `plan_class_schedule` groups a softmax round's classes. How many classes
  ...
  or `MOJOTREES_GPU_CLASS_BATCH` asks for more.
...
Environment
-----------
`MOJOTREES_GPU_HIST_SPECIALIZATION`, following the `MOJOTREES_` contract in
`parallel.mojo`: `off` or `baseline` (the default, and the value of anything
unrecognized), `shape`, `packed`, `batched`. The existing
`MOJOTREES_GPU_ROW_TILE` and `MOJOTREES_GPU_BLOCK_THREADS` overrides are
...
def env_specialization_level() -> Int:
    """`MOJOTREES_GPU_HIST_SPECIALIZATION` as a level constant.
    ...
    """
    var s = getenv("MOJOTREES_GPU_HIST_SPECIALIZATION")
    if s == "shape":
...
def profile_from_caps(caps: DeviceCaps) -> GpuProfile:
    ...
    either way. Once a budget accessor exists, a call site that knows it is on
...
def derive_histogram_plan(
    ...
    `MOJOTREES_GPU_HIST_SPECIALIZATION` and then defaults to
    ...
    if applied >= SPEC_LEVEL_SHAPE:
        ...
        #
        # The `MOJOTREES_GPU_ROW_TILE` and `MOJOTREES_GPU_HIST_STRATEGY`
...
struct ClassSchedule(Copyable, Movable):
    ...
    def is_sequential(self) -> Bool:
        ...
        batch of one unless a caller or `MOJOTREES_GPU_CLASS_BATCH` asks for
        more."""
...
def plan_class_schedule(
    ...
    **The default is the sequential path.** A `requested_batch` of zero
    consults `MOJOTREES_GPU_CLASS_BATCH` and then asks for a batch of one,
