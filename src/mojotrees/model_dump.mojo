...
`bindings/_mojotrees.mojo` hands them to Python, and
`docs/MODEL_INSPECTION_SCHEMA.md` is the normative statement both answer
to.

What lives here, and why it is not in a consumer
------------------------------------------------
Everything that has to know how a mojotrees tree is laid out: node depths
...
def threshold_value(
    ...
    mojotrees splits on bin ids, and a value maps to the first bin whose
...
def dump_raw_scores(
    ...
    arithmetic. mojotrees stores unshrunk leaf values and multiplies by the
