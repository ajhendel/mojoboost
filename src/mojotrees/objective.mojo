...
`bindings/_mojotrees.mojo` and `bench/bench_custom_objective.py`): the cost
...
- Multiclass. Custom objectives are single-output only. LightGBM allows a
  ...
  matrix; mojotrees does not, and `train_multiclass` has no custom-objective
...
def apply_sample_weight(
    ...
    class-level policy mojotrees has (`class_weight`, `balanced`,
