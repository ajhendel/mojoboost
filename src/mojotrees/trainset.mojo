...
struct Dataset(Copyable, Movable, Writable):
    ...
    def matches_binning(self, other: Dataset) raises -> Bool:
        ...
        `raises` because `BinMapper.matches` does: it compares the fitted CTR
        tables, which are walked and indexed. Both callers are tests and both
        already raise, so this widens no contract. It was missing, and the
        package did not elaborate with it missing.
        """
