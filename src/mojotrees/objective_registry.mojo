...
bindings/_mojotrees.mojo, and `_METRICS` / `_ALIASES` / `_DEFAULTS` in
python/mojotrees/_eval.py. The Python tables are the ones that hurt: they
...
# bindings/_mojotrees.mojo takes by hand.
comptime TRANSFORM_OBJECTIVE_LINK = 0
comptime TRANSFORM_SOFTMAX = 1
comptime TRANSFORM_RAW = 2

# How a name a user typed relates to what mojotrees implements.
...
# `eval_metric` in bindings/_mojotrees.mojo dispatches on and
# python/mojotrees/_eval.py mirrors, and must not be renumbered: a code
...
def objective_is_known(objective: Int) -> Bool:
    """Whether this code names any objective mojotrees has: the built-ins
...
def objective_canonical_name(objective: Int) raises -> String:
    """The LightGBM name mojotrees reports this objective under.
...
def objective_unimplemented_canonical(name: String) -> String:
    """The primary spelling of a LightGBM objective mojotrees has not
...
def objective_unimplemented_reason(name: String) -> String:
    """Why a LightGBM objective mojotrees has not implemented is a
    ...
    python/mojotrees/__init__.py repeats in its own words. Each is also a
    ...
    implemented" from a parameter string and as "use MojoTreesClassifier"
    from an estimator.
    """
    if name == "cross_entropy_lambda" or name == "xentlambda":
        return String(
...
def objective_code_from_name(name: String) raises -> Int:
    ...
    A LightGBM objective mojotrees has not implemented is reported by name
...
def objective_name_status(name: String) -> Int:
    """Whether a name is supported, is a real LightGBM objective mojotrees
...
# Every weighting policy mojotrees has ends as one `sample_weight` vector,
...
def metric_canonical_name(metric: Int) raises -> String:
    """The LightGBM name mojotrees reports this metric under."""
    if metric == METRIC_L2:
        return String("l2")
