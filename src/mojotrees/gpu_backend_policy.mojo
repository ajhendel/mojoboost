...
- `device_policy.env_declared_api()` reads `MOJOTREES_GPU_BACKEND`, the
...
Environment contract, following `MOJOTREES_GPU_TRANSFER_UNPROVEN` in
`unified_memory_policy.mojo`, which is the same shape of gate:

- `MOJOTREES_GPU_BACKEND_UNVALIDATED=1` acknowledges that a specialized path
...
(`cuda`) device types, so its user chooses the backend. mojotrees has one
...
def backend_support(api: Int) -> Int:
    ...
    ships today, where nobody sets `MOJOTREES_GPU_BACKEND` and no reported
...
def backend_is_identified(api: Int) -> Bool:
    ...
    `MOJOTREES_GPU_BACKEND` is unset on an ordinary run. It is the reason
...
def env_ack_unvalidated() -> Bool:
    """Whether `MOJOTREES_GPU_BACKEND_UNVALIDATED=1` acknowledges running a
    specialized path on a backend with no validation record."""
    return getenv("MOJOTREES_GPU_BACKEND_UNVALIDATED") == "1"
...
def require_backend_exercised(api: Int, what: String) raises:
    ...
    The escape hatch is `MOJOTREES_GPU_BACKEND_UNVALIDATED=1`, and it exists
    for the same reason `MOJOTREES_GPU_TRANSFER_UNPROVEN=1` does in
    ...
    **An unidentified backend is not refused**, and that is the honest limit
    ...
    an API name before it launches, and `MOJOTREES_GPU_BACKEND` is unset on
    ...
    raise Error(
        ...
        " specializing for it; set MOJOTREES_GPU_BACKEND_UNVALIDATED=1 to"
