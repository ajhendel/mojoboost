...
unless `MOJOBOOST_GPU_BACKEND_UNVALIDATED=1` acknowledges it.
...
arbitrarily. mojoboost has one source and one `gpu` device value, so a
...
def derive_plan_for(
    ...
    Everything else, including the `MOJOBOOST_GPU_ROW_TILE`,
    `MOJOBOOST_GPU_BLOCK_THREADS`, and `MOJOBOOST_GPU_HIST_STRATEGY`
...
def require_cuda(api: Int) raises:
    ...
    `MOJOBOOST_GPU_BACKEND=cuda`, which `device_policy.env_declared_api`
    reads.
    """
    if api == API_CUDA:
        return
    raise Error(
        ...
        " gpu_tiling.mojo; declare MOJOBOOST_GPU_BACKEND=cuda if this"
