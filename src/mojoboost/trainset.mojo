...
(The neighboring `dataset.mojo` is a different thing despite the name: it
holds `RawData`, the dense-or-sparse raw input type that binning consumes.
This module is the LightGBM-facing training set built on top of it.)
...
def update_dataset_multiclass(
    ...
    Continued training runs on the CPU, as `update_dataset` does.
    """
    if not model.mapper.matches(dataset.mapper):
        raise Error(
            "continued training needs the dataset the model was trained on:"
