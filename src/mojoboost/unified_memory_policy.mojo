...
**Claim 2**, no duplication for our data: the buffer mojoboost fills on the
...
- **No timing.** Nothing here reads a clock or holds a measurement. A route
  ...
  handed.
- **No inference from `no_copy_issued`.** That flag means mojoboost enqueued
...
Environment contract, matching the `MOJOBOOST_` convention:

- `MOJOBOOST_GPU_TRANSFER`: `staged` (the default), `direct`, `mapped`,
...
- `MOJOBOOST_GPU_TRANSFER_UNPROVEN=1`: acknowledge that the requested route
...
"""mojoboost enqueued no copy. A fact about this library, not about the
...
struct Footprint(Copyable, Movable):
    ...
    what mojoboost asked the runtime for. What the system committed, what it
...
struct RouteDecision(Copyable, Movable):
    ...
    """True when the selection rests on `MOJOBOOST_GPU_TRANSFER_UNPROVEN=1`
...
def route_block_reason(
    ...
    2. Structural support (`structural_support`). A route that cannot work
       given how mojoboost owns its data is refused here, and no measurement
...
def env_requested_route() raises -> Int:
    """`MOJOBOOST_GPU_TRANSFER`, defaulting to the staged copy.
    ...
    """
    var s = getenv("MOJOBOOST_GPU_TRANSFER")
    if s.byte_length() == 0:
...
def env_ack_unproven() -> Bool:
    """`MOJOBOOST_GPU_TRANSFER_UNPROVEN=1`."""
    return getenv("MOJOBOOST_GPU_TRANSFER_UNPROVEN") == "1"
