...
- **Asymmetric cores, handled conservatively.** Apple silicon reports a mix
  ...
  and no measurement here justifies it. `MOJOBOOST_CPU_CORE_POOL=performance`
...
--------------------------------------------------------
- `MOJOBOOST_CPU_TASKS_PER_CORE`: positive integer, auto-mode fan-out per
  core. Default `DEFAULT_TASKS_PER_CORE`.
- `MOJOBOOST_CPU_CORE_POOL`: `all` (default) counts every physical core;
  `performance` counts only the reported performance cores.
- `MOJOBOOST_CPU_FEATURE_GROUP`: `1` or `2`, how many features one
  accumulation loop interleaves. Default `DEFAULT_FEATURE_GROUP`, further
  clamped by the L1 estimate.
- `MOJOBOOST_CPU_COMPACT_MIN_ROWS`: row count below which a subset
  accumulation skips the gradient/hessian gather. Default
  `DEFAULT_COMPACT_MIN_ROWS`.

`MOJOBOOST_NUM_WORKERS` and `MOJOBOOST_PARALLEL_MIN_OPS` keep their meanings
...
def env_tasks_per_core() -> Int:
    ...
    return _positive_or(
        _env_int("MOJOBOOST_CPU_TASKS_PER_CORE", DEFAULT_TASKS_PER_CORE),
...
def env_core_pool() -> Int:
    ...
    var s = getenv("MOJOBOOST_CPU_CORE_POOL")
...
def env_feature_group() -> Int:
    return _env_int("MOJOBOOST_CPU_FEATURE_GROUP", DEFAULT_FEATURE_GROUP)


def env_compact_min_rows() -> Int:
    return _env_int("MOJOBOOST_CPU_COMPACT_MIN_ROWS", DEFAULT_COMPACT_MIN_ROWS)
...
struct CpuProfile(Copyable, Movable):
    ...
    def dispatch_cores(self) -> Int:
        """Cores auto mode plans against, under `MOJOBOOST_CPU_CORE_POOL`."""
