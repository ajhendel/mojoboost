...
Every trainer, dataset constructor, serializer, and binding in mojoboost
...
mojoboost structs, and imports nothing from the package. That is deliberate:
...
def check_features_finite(
    ...
    NaN is mojoboost's missing-value marker: the binner excludes it from the
...
struct ColumnReport(Copyable, Movable):
    ...
    mojoboost. What it is, is the explanation a caller needs when a feature
...
def check_classes_present(codes: List[Int], n_classes: Int) raises:
    ...
    accepts this; mojoboost names the missing class instead, because the
...
def check_csc(
    ...
    """CSC in the orientation mojoboost stores it: features compressed, rows
    indexed."""
    check_compressed(
        col_offsets,
        row_index,
...
def check_tree_topology(
    ...
    function exists to close. Every grower in mojoboost appends nodes as it
