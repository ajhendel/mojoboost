...
- LightGBM decides that constraints are in play by checking whether the
  vector is empty. mojoboost also treats an all-zero vector as inactive,
...
- Quantile and L1 leaf renewal replaces every leaf's Newton value with a
  ...
  monotonicity. mojoboost clamps the renewed value back into the leaf's
  ...
  mojoboost-defined rather than matched: it biases the renewed quantile
