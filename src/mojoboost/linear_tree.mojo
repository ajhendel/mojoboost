...
mojoboost-defined; it is not read off LightGBM. It is why `center` is stored
...
- **Regularization placement.** `linear_lambda` is applied to the
  ...
  mojoboost choice and flagged in `docs/LINEAR_TREES.md` as parity-unverified.
- **Missing values.** The eligibility rule and the centre substitution above
  are mojoboost-defined. LightGBM's behaviour here has not been read off its
  source.
...
struct LinearParams(Copyable, Movable):
    ...
    - `max_leaf_features`: cap on how many features one leaf may use, -1 for
      no cap. mojoboost extension.
    - `min_data_per_linear_feature`: a leaf must hold at least this many rows
      per fitted feature. 0 is off. mojoboost extension.
    - `ridge_eps`: relative diagonal jitter, `eps * trace(A) / m`, so a
      singular system with `linear_lambda = 0` still factorizes. mojoboost
      extension.
    - `max_linear_deviation`: discard a fit whose linear part exceeds this in
      ...
      check that costs a second pass over the leaf's rows. mojoboost
      extension.
    """
...
def check_objective_compatible(objective: Int) raises:
    ...
    if objective == _LAMBDARANK:
        raise Error(
            ...
            " is not a combination LightGBM documents or mojoboost has"
            " checked"
        )
