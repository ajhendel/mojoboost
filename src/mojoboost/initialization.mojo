...
| --- | --- | --- |
| `py_import` | executing `mojoboost/__init__.py` and its module graph | deferring imports |
| `ext_load` | `dlopen` of `_mojoboost.so` and its dependency closure | fewer/smaller dylibs, better rpaths |
...
Environment contract, matching the `MOJOBOOST_` convention in
parallel.mojo, gpu_tiling.mojo, and gpu_runtime.mojo:

- `MOJOBOOST_STARTUP_TRACE=1` turns on startup timing. Off by default, so
  an untraced process reads no clock.
- `MOJOBOOST_GPU_WARMUP`: `off` (default), `train`, or `all`. Selects how
...
- `MOJOBOOST_STARTUP_REPORT_FD`: reserved, unread here. See the handoff;
...
def phase_name(phase: Int) -> String:
    ...
    docs/STARTUP_LATENCY.md and python/mojoboost/diagnostics.py both
...
struct StartupTrace(Copyable, Movable):
    ...
    def from_env() -> StartupTrace:
        """A trace configured by `MOJOBOOST_STARTUP_TRACE`.
        ...
        """
        return StartupTrace(getenv("MOJOBOOST_STARTUP_TRACE") == "1")
    ...
    def report(self) -> String:
        ...
        python/mojoboost/diagnostics.py parses exactly this and is the
...
def env_warmup_level() -> Int:
    """`MOJOBOOST_GPU_WARMUP` as a level. Unset or unrecognized is
    `WARMUP_OFF`, so an unknown value never silently front-loads work."""
    var s = getenv("MOJOBOOST_GPU_WARMUP")
    if s == "train":
        return WARMUP_TRAIN
    if s == "all":
        return WARMUP_ALL
