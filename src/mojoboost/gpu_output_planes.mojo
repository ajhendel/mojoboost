...
mojoboost but `parallel._env_int`.
...
# count. `MOJOBOOST_GPU_CLASS_BATCH_BYTES` overrides it.
...
def env_class_batch() -> Int:
    """`MOJOBOOST_GPU_CLASS_BATCH`: force a batch size, 0 for auto. Matches
    the `MOJOBOOST_` override contract in `parallel.mojo` and
    `gpu_tiling.mojo`, and exists for the same reason: a benchmark has to be
    able to pin the geometry a measurement was taken at."""
    return _env_int("MOJOBOOST_GPU_CLASS_BATCH", 0)


def env_class_batch_budget() -> Int:
    """`MOJOBOOST_GPU_CLASS_BATCH_BYTES`, defaulting to
    `CLASS_BATCH_BUDGET_BYTES`."""
    return _env_int(
        "MOJOBOOST_GPU_CLASS_BATCH_BYTES", CLASS_BATCH_BUDGET_BYTES
    )
...
def plan_class_batches(
    ...
    `budget_bytes` of 0 reads `MOJOBOOST_GPU_CLASS_BATCH_BYTES` and then the
    default budget; `requested_batch` of 0 reads `MOJOBOOST_GPU_CLASS_BATCH`
    ...
    if forced > 0:
        ...
        if cost > budget:
            raise Error(
                ...
                " raise MOJOBOOST_GPU_CLASS_BATCH_BYTES or lower"
                " MOJOBOOST_GPU_CLASS_BATCH"
            )
    else:
        batch = n_classes
        while batch > 1:
