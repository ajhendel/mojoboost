...
two prediction bounds `lower_bound` and `upper_bound`. mojoboost has had
...
----------------------------
A fitted mojoboost model is a set of claims. An edit is safe exactly when
...
# covers and which it does not, and `python/mojoboost/inspection.py` mirrors
...
def leaf_node_index(tree: Tree, leaf_ordinal: Int) raises -> Int:
    ...
    Leaf ordinals are mojoboost's own numbering (`Tree.leaf_ordinals`): a
...
def leaf_outputs(tree: Tree) -> List[Float64]:
    ...
    These are **unshrunk**: mojoboost stores the value a leaf was fitted at
...
def get_leaf_output(
    ...
    mojoboost's leaf numbering and mojoboost's value scale (see
...
def set_leaf_output(
    ...
    - the leaf is addressed by mojoboost's leaf ordinal, not LightGBM's leaf
...
def raw_score_bounds(booster: Booster) raises -> ScoreBounds:
    ...
    it there too; mojoboost keeps it apart, so it is added here explicitly
...
def editing_capabilities() -> List[EditingCapability]:
    ...
    out.append(
        EditingCapability(
            ...
            True,
            String(
                "the stored, unshrunk value, addressed by mojoboost's leaf"
