...
unexercised backend unless `MOJOBOOST_GPU_BACKEND_UNVALIDATED=1`
acknowledges it.
...
mojoboost has one source for Metal, CUDA, and HIP alike, so an AMD
...
def require_amd(api: Int) raises:
    ...
    `MOJOBOOST_GPU_BACKEND=hip`, which `device_policy.env_declared_api`
    ...
    raise Error(
        ...
        " gpu_tiling.mojo; declare MOJOBOOST_GPU_BACKEND=hip if this device"
        " is known to be AMD",
    )
