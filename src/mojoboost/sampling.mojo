...
mojoboost grows leaf-wise, so a "level" is a node's depth in edges from the
...
spelling of a row or feature sampling parameter onto the mojoboost name,
including `colsample_bytree` and `colsample_bynode`, which the parity audit
records as spellings mojoboost does not yet accept. The table here is meant
...
an empty list means "every row" everywhere in mojoboost. `contiguous_ranges`
...
def sampling_param_names() -> List[String]:
    """Every sampling parameter mojoboost names, in mojoboost's spelling.
...
def canonical_sampling_param(name: String) raises -> String:
    """The mojoboost name for one sampling parameter spelling.
    ...
    fraction mojoboost adds. Raises on anything else rather than passing an
...
def canonical_data_sample_strategy(value: String) raises -> String:
    ...
    gradient-based one-side sampling (goss.mojo). mojoboost also reaches GOSS
...
def check_row_set(rows: List[Int], n_rows: Int) raises:
    ...
    Every sampled row set mojoboost builds is ascending and duplicate free
