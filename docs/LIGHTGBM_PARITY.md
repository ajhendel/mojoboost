...
| Level-wise GPU growth | deferred | yes | no | no | no | n/a | n/a | n/a | `src/mojotrees/gpu_levelwise.mojo` with its rule table in `src/mojotrees/levelwise_policy.mojo`. No trainer offers a level-wise mode, so nothing reaches either, and growth stays leaf-wise everywhere. No suite imports them |
