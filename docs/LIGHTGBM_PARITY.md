...
| `max_cat_to_onehot` | partial | Categorical. **Off by one against LightGBM, verified from source 2026-08-16.** LightGBM's test is `num_bin <= max_cat_to_onehot` (`src/treelearner/feature_histogram.cpp:183`), where `num_bin` is `kept_categories + 1` because `src/io/bin.cpp:456-459` pushes a dummy bin at index 0. So LightGBM one-hots `max_cat_to_onehot - 1` real categories and mojotrees one-hots `max_cat_to_onehot` -- at the shared default of 4, they one-hot 3 and we one-hot 4. Owned by the `wide-categorical-bins` lane, which is blocked on nothing now; see `docs/design/CATBOOST_CATALOG.md` A16, which also records why CatBoost's `one_hot_max_size` is a **different** boundary (`<=` on real categories, no `+1`, with a `>= 2` lower guard) and must not be collapsed onto this one |
