...
| GPU speed vs CPU | different | Measured, and the answer depends on the shape. On an Apple M4 at 100 rounds, 31 leaves, 255 bins, squared error, training time with binning excluded: 1,000,000 x 50 is CPU 6.98s against GPU 3.58s, so the GPU is 1.85x; 250,000 x 50 is CPU 1.66s against GPU 1.89s; 50,000 x 50 is CPU 0.564s against GPU 1.63s. The device carries roughly 1.5s of fixed cost per fit, so it loses to our own CPU below about a million rows. Multiclass is the other way round: 465,000 x 54 over 7 classes is CPU 25.47s against GPU 15.30s, so the GPU wins by 1.63x. `crossover_rules()` now holds one rule from those records, scoped to Metal on an M4 for unweighted squared error at 1,000,000 rows by 50 features and above, but it cannot fire through `resolve_device` because `DeviceCapabilities.detect()` opens no device and a hardware-scoped rule cannot match an unidentified profile, so a defaulted `fit(device='auto')` still gets the CPU with a warning. `MOJOTREES_AUTO_MIN_CELLS` remains the escape hatch and reports `EVIDENCE_ENV` rather than a validated choice. `bench/results/profile_2026-08-15/RESULTS.md`, `docs/GPU_VALIDATION.md` |
