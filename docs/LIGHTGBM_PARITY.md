...
| `feature_pre_filter` | different | **The one stock default mojotrees does not match, deliberately.** LightGBM defaults this to `true`, and `true` is a Dataset construction step, not a training setting: `BinMapper::FindBin` marks a feature trivial when `NeedFilter(cnt_in_bin, total_sample_cnt, min_split_data, ...)` finds no prefix of its bins that leaves at least `min_split_data` rows on both sides, and `Dataset::Construct` then leaves every trivial feature out of `used_features` (`src/io/bin.cpp`, verified; `min_split_data` is `min_data_in_leaf` scaled to the sample in `src/io/dataset_loader.cpp`, which was not read for this row). A dropped feature is gone from the matrix, so it is also gone from the pool `feature_fraction` samples and from every feature index and importance vector. Accepting the flag without doing the deletion would leave the two libraries fitting **different feature spaces**, which is the same error EFB is held back for, so `tree_parameters_extra.check_feature_pre_filter` still accepts `false` and still refuses `true`, and `tools/check_parity.py` fails if that refusal is removed. What is missing to reach `true` honestly: per-bin row counts kept out of the fit, a `NeedFilter` test on them, a trivial-feature flag on `BinMapper`, and a filtered feature pool threaded through the trainers and the feature-fraction sampler. Until then, a differential against LightGBM must pin `feature_pre_filter=False` on the LightGBM side or it is comparing two feature spaces |
