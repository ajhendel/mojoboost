...
| Depth-wise (level-wise) growth | different | yes | yes | yes | yes | n/a | n/a | n/a | `TreeParams.grow_policy` / Python `grow_policy=` / parameter string `grow_policy` (`leafwise` default, `depthwise`; XGBoost's names, LightGBM has no such switch). Order decided by `src/mojotrees/levelwise_policy.mojo` `LevelSchedule`, followed by `tree.grow_tree` (`src/mojotrees/tree.mojo`), `tree_sparse.grow_tree_sparse` (`src/mojotrees/tree_sparse.mojo`), and all three loops in `src/mojotrees/train_gpu.mojo`; the distributed prototype rejects it. `tests/test_grow_policy.mojo`. The batched per-level device launch the design doc argues for (`src/mojotrees/gpu_levelwise.mojo`, `docs/design/GPU_LEVELWISE.md`) is still unbuilt: depth-wise on the GPU costs the same launches as leaf-wise today |
...
| Hybrid CPU and GPU leaf placement | deferred | yes | no | no | no | n/a | n/a | n/a | `src/mojotrees/hybrid_leaf_scheduler.mojo` decides per leaf which device should build its histogram, over the cache model in `src/mojotrees/histogram_cache_policy.mojo`. Both modules are reached as of this revision and neither places a leaf: `src/mojotrees/gpu_runtime.mojo` imports the scheduler, and `GpuSession.note_hybrid` builds a `HybridContext.from_env` so a run can report what `MOJOTREES_HYBRID_LEAVES` and `MOJOTREES_HYBRID_TRACE` asked for and why it was declined. The file says it plainly: "Nothing here moves a histogram." Every histogram is still built where it would have been built. No suite imports either module |
| GPU sparse and categorical kernels | deferred | yes | no | no | no | n/a | n/a | n/a | `src/mojotrees/gpu_categorical.mojo`, `src/mojotrees/gpu_sparse.mojo`, and `src/mojotrees/gpu_sparse_layout.mojo` exist as a chain that no root reaches. The GPU trainer still refuses both sparse input and categorical features by name, which section 12 records, so the refusal a user meets is accurate; what is not accurate is calling the kernels absent. No suite imports any of the three |
| DART and random forest boosting | deferred | yes | no | no | no | n/a | n/a | n/a | `src/mojotrees/alternate_boosting.mojo` dispatches to `src/mojotrees/boosting_dart.mojo` and `src/mojotrees/boosting_rf.mojo`. Nothing imports the dispatcher, so `src/mojotrees/boosting.mojo` still owns the only round loop and `boosting="dart"` and `"rf"` remain unrecognized. No suite imports any of the three |
| Cost-effective gradient boosting | deferred | yes | no | no | no | n/a | n/a | n/a | `src/mojotrees/cegb.mojo` implements LightGBM's four `cegb_*` controls as a gain adjustment, and `src/mojotrees/params.mojo` parses `cegb_tradeoff` and `cegb_penalty_split`. No grower charges a candidate, so the parsed values reach a config field nothing reads. No root reaches the module and no suite imports it |
...
| GPU prediction | partial | yes | yes | yes | yes | yes | yes | yes | `src/mojotrees/gpu_predict.mojo`, reached from `Model.predict_batch(device=)` in the Mojo API. The Python `predict` binding takes no device (`bindings/_mojotrees.mojo`), so the estimators always predict on the CPU |
...
| `subsample_for_bin` | deferred | mojotrees bins from every row rather than a sample. Correct but slower on very large data; the sampled binner is a performance item, not a semantic one |
...
| `colsample_bytree` | partial | mojotrees's name is `feature_fraction`; `colsample_bytree` itself is **not** accepted as an alias, and neither is `colsample_bynode`, though `feature_fraction_bynode` is spelled natively. Confirmed against `_Base.__init__` in this audit. Adding the two `colsample_*` spellings is a v1 gap |
| `reg_alpha` | supported | Alias for `lambda_l1` |
| `reg_lambda` | supported | Alias for `lambda_l2`. Note the default differs: mojotrees defaults to 1.0, LightGBM to 0.0. Documented in the README defaults table |
...
| `tree_learner` | different | mojotrees has one grower. The distributed prototype is selected by calling `train_distributed`, not by a string |
| `num_threads` | different | `MOJOTREES_NUM_WORKERS`, see section 2 |
| `device_type` | supported | Accepted as `device` with `device_type` as an alias. Values differ: mojotrees has `cpu`, `gpu`, `auto`; LightGBM has `cpu`, `gpu`, `cuda`. One portable GPU backend, so no vendor split, and `auto` is an addition. `gpu` now covers multiclass as well as single-output training (`gpu_supports` in `src/mojotrees/device.mojo` admits every output count); the module docstring above it still says multiclass is CPU only and is stale, which is recorded in "Known gaps" |
...
| `cegb_tradeoff` / `cegb_penalty_split` / `cegb_penalty_feature_lazy` / `cegb_penalty_feature_coupled` | deferred | Cost-effective gradient boosting. All four now have an implementation in `src/mojotrees/cegb.mojo`, including the lazy per-feature penalty and the per-row read state it needs, and no root reaches that module. `src/mojotrees/params.mojo` parses `cegb_tradeoff` and `cegb_penalty_split` into a config field no grower reads, so setting either still does nothing. Section 0 |
...
| `force_col_wise` / `force_row_wise` | different | mojotrees builds histograms feature-major, always. The choice exists in LightGBM to trade off multi-threading strategies; here the parallel dispatch is governed by `MOJOTREES_PARALLEL_MIN_OPS` |
...
| GPU prediction | partial | `Model.predict_batch` and `MulticlassModel.predict_batch` take a `device` and walk the trees on the accelerator; binning stays host-side, so both devices route every row to the same leaf. Mojo API only: the `predict` binding takes no device, so the Python estimators always predict on the CPU. Not a LightGBM capability (`device_type` covers training only), so this is a mojotrees addition. `src/mojotrees/gpu_predict.mojo`, `tests/parallel/test_gpu_predict.mojo` |
| Persistent GPU session and scheduling | partial | `src/mojotrees/gpu_runtime.mojo` is exported and `src/mojotrees/histogram_gpu.mojo` borrows a `GpuSession`'s device context. The residency ledger, staging ring, and phase counters around it are exercised by `tests/parallel/test_gpu_runtime.mojo` rather than by a trainer |
