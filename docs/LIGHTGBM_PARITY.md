...
| `subsample_for_bin` | deferred | The sampled edge fit exists as `fit_bins(..., bin_construct_sample_cnt=...)` in `src/mojotrees/binning.mojo` (section 5), but no estimator parameter reaches it: `MojoTreesRegressor(subsample_for_bin=...)` is not accepted, so from the scikit-learn API mojotrees still bins from every row |
...
| `min_data_in_bin` | partial | A `fit_bins` argument (`src/mojotrees/binning.mojo`), honored in both branches of LightGBM's `GreedyFindBin`, verified against `src/io/bin.cpp` on LightGBM master: with no more distinct values than ordinary bins the levels are merged in order until each bin holds the minimum, and past the bin budget the budget itself shrinks to `max(1, min(max_bin, n / min_data_in_bin))`. Also enforced for categorical features (`min_data_per_group` governs the sorted search). `partial` for three reasons: the default here is 1, not LightGBM's 3, because 1 is the setting at which the binner is the binner it was; the per-level greedy LightGBM runs *inside* the shrunken budget (its `is_big_count_value` rule and its recomputed target bin size) is still not implemented, so the two diverge on a column with more distinct values than bins; and the sparse binner (`sparse.fit_bins_csc`) does not take the argument. No parameter-string key and no Python estimator parameter reaches it |
| `data_random_seed` | partial | A `fit_bins` argument seeding the `bin_construct_sample_cnt` sample below. Unused at the default, where binning reads every row and is deterministic without a seed. The stream is mojotrees's own counter-based splitmix64, not LightGBM's 32-bit LCG, so the same seed does not select the same rows |
| `bin_construct_sample_cnt` | partial | A `fit_bins` argument (`src/mojotrees/binning.mojo`): the bin edges are fit from a sample of the rows drawn once per fit and shared by every feature, as LightGBM's are. The sample is exactly `bin_construct_sample_cnt` ascending row indices from a counter-based splitmix64 selection sample, identical at every `MOJOTREES_NUM_WORKERS`. `partial` because the default is 0, meaning every row, rather than LightGBM's 200,000; because the sample is not LightGBM's sample for a given seed; and because no parameter-string key, no Python estimator parameter, and not the sparse binner reaches it. See `subsample_for_bin` in section 2 |
...
| `feature_pre_filter` | different | mojotrees does not drop features during binning, so there is nothing to disable. A constant feature simply never yields a positive-gain split. `false` is therefore not an unimplemented option, it is the option mojotrees implements, and `tree_parameters_extra.check_feature_pre_filter` accepts it rather than refusing the key by name for any value as it used to; a LightGBM configuration that spells out the setting mojotrees matches now ports across unchanged. `true` is still refused, and not only for the missing speed-up: dropping a feature from the Dataset also drops it from the pool `feature_fraction` samples, so a prefiltered fit can grow different trees rather than the same trees faster, and LightGBM additionally forbids reusing a prefiltered Dataset with a larger `min_data_in_leaf` |
