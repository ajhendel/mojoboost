...
file was written)
Audited: 2026-08-14, mojoboost at commit `ab25ad1` plus the working tree of
that day
...
8, and 9, which are carried forward from the version 1 audit against 4.7.0
and were not re-enumerated against a running install in this pass.
...
| Model inspection and dump (`mojoboost.inspection`) | partial | yes | yes | yes | yes | no | n/a | yes | `dump_model`, `trees_to_dataframe`, `trees_to_records`, and `get_split_value_histogram` are in `mojoboost.__all__` and resolve the submodule lazily, so all four are public. `partial` is now about the *producer*, not the reach: `python/mojoboost/inspection.py` rebuilds the schema by parsing `Booster.model_to_string()`, while `src/mojoboost/inspection.mojo` and `model_dump.mojo` are the native producers with no caller. `bindings/inspection_bindings.mojo` implements `dump_model` and five more entry points and `bindings/_mojoboost.mojo` registers none of them, so the native path is written on both sides of a seam nobody has closed. `python/tests/parallel/test_inspection.py`, and `docs/INTEGRATION_INVENTORY.md` |
| Split gains in a dump | deferred | yes | no | no | no | n/a | n/a | no | `src/mojoboost/serialize.mojo` does not write split gains (see the comment at the leaf reader), so every dumped node carries `split_gain: None` and `has_split_gain: False`. `python/mojoboost/inspection.py` picks gains up automatically from a `_mojoboost.split_gains` hook. Nothing defines one: unlike the other inspection entry points, `split_gains` appears nowhere in `bindings/`, so this is a missing implementation rather than an unregistered one |
...
| Remaining tree-parameter rules | deferred | yes | no | no | yes | n/a | n/a | n/a | `src/mojoboost/tree_parameters_extra.mojo` holds validated rules for `min_gain_to_split`, `max_delta_step`, `path_smooth`, `feature_contri`, the computable CEGB penalties, `extra_trees`, `monotone_penalty`, and parsed forced splits, with `tests/parallel/test_tree_parameters_extra.mojo`. No grower calls any of them, so setting one of those parameters still does nothing |
| Apple GPU tuning policy | deferred | yes | no | no | yes | n/a | yes | n/a | `src/mojoboost/apple_gpu_policy.mojo` with `tests/parallel/test_apple_gpu_policy.mojo`. Nothing reads it; `src/mojoboost/gpu_tiling.mojo` is still the tiling policy in force |
| Per-level feature sampling | deferred | yes | no | no | yes | n/a | n/a | n/a | `select_level_features` and `select_split_features` in `src/mojoboost/sampling.mojo`, with `tests/parallel/test_sampling.mojo`. Neither is exported nor called by any grower, which reach for `select_tree_features` and `select_node_features`. It is XGBoost's `colsample_bylevel`, so it is an extension rather than a parity row |
...
| GPU split selection on the device | supported | yes | yes | yes | yes | yes | yes | yes | `src/mojoboost/gpu_split_search.mojo`, reached from `grow_tree_gpu` and from `MOJOBOOST_GPU_SPLIT_STRATEGY=device`. Off by default: `SPLIT_SEARCH_AUTO` resolves to the host scan, because Float32 device gains can flip near-tie decisions and no benchmark has compared the two |
...
| `Booster.save_model` / `model_to_string` / `model_from_string` | different | Present under LightGBM's names, but the format is mojoboost's own versioned text (`src/mojoboost/serialize.mojo`, now at v3), which stores floats as raw bit patterns so a round trip predicts bit-exactly. It is not LightGBM-readable; `src/mojoboost/lgbm_model_io.mojo` is an unintegrated experiment in reading LightGBM's format, section 0. `save_model` takes no `num_iteration` or `start_iteration` |
| `Booster.dump_model` / `trees_to_dataframe` | partial | `mojoboost.inspection.dump_model(model)` and `trees_to_dataframe(model)` build a documented, versioned schema (`docs/MODEL_INSPECTION_SCHEMA.md`) from `Booster.model_to_string()`. Two gaps: they are module functions rather than `Booster` methods, and split gains are not in the model text, so every node reports `split_gain: None` and the dump reports `has_split_gain: False`. Section 0 |
...
| `Booster.get_leaf_output` / `set_leaf_output` / `shuffle_models` / `refit` | deferred | Model editing. `python/mojoboost/inspection.py` states the reason it is read only: a fitted tree carries node covers, internal node values, and split gains that were computed at growth time, and an arbitrary leaf edit falsifies its ancestors while leaving them in place, with no check able to tell an intentional edit from a corrupt one. Task 14 |
...
| `extra_trees` / `extra_seed` | deferred | Randomized split thresholds. The decision rule exists in `src/mojoboost/tree_parameters_extra.mojo` and no grower calls it, so setting it does nothing today. Section 0, task 12 |
...
| `enable_bundle` | deferred | Exclusive Feature Bundling. `src/mojoboost/efb.mojo` implements the core and nothing enables it. Section 0, task 13 |
