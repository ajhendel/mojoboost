...
file was written)
Audited: 2026-08-14, mojoboost at commit `860b1cf` plus the working tree of
that day
...
| GPU packed-bin layout | deferred | yes | no | no | no | n/a | n/a | n/a | `src/mojoboost/gpu_binned_layout.mojo` plans a packed layout over the bit primitives in `src/mojoboost/gpu_bin_packing.mojo`. `train_gpu` never asks for a plan, so no root reaches either module and no suite imports them. `docs/INTEGRATION_INVENTORY.md` |
...
| Class-batched GPU multiclass rounds | deferred | yes | no | no | no | n/a | n/a | n/a | `src/mojoboost/gpu_multiclass_batch.mojo` over the interleaved planes in `src/mojoboost/gpu_output_planes.mojo`. GPU multiclass training is one tree per class per round today (`train_multiclass_gpu`), which is what the supported row below describes; nothing reaches the batched path and no suite imports it |
| Hybrid CPU and GPU leaf placement | deferred | yes | no | no | no | n/a | n/a | n/a | `src/mojoboost/hybrid_leaf_scheduler.mojo` decides per leaf which device should build its histogram, over the cache model in `src/mojoboost/histogram_cache_policy.mojo`. No trainer consults it. `MOJOBOOST_HYBRID_LEAVES` and `MOJOBOOST_HYBRID_TRACE` name a scheduler nothing runs. No suite imports either module |
...
- **v3 (2026-08-14)**: re-audited reachability against the tree at
  ...
  reachable` cells cannot rot the way those three did.
