...
| DART and random forest boosting | deferred | yes | no | no | no | n/a | n/a | n/a | `src/mojotrees/alternate_boosting.mojo` dispatches to `src/mojotrees/boosting_dart.mojo` and `src/mojotrees/boosting_rf.mojo`. Nothing imports the dispatcher, so `src/mojotrees/boosting.mojo` still owns the only round loop reachable from a root and `boosting="dart"` and `"rf"` remain unrecognized. Both modes are now complete behind that wall: single-output, multiclass, continuation, and validation-set early stopping, with the DART drop rules read off LightGBM's `src/boosting/dart.hpp` (both `uniform_drop` settings, `docs/DART.md` section 4.1). Compiled, never run; no suite imports any of the three |
...
| `drop_rate` / `max_drop` / `skip_drop` / `xgboost_dart_mode` / `uniform_drop` / `drop_seed` | deferred | DART parameters. All six are implemented on `boosting_dart.DartParams` at LightGBM's defaults and semantics (`docs/DART.md`), but no parameter name reaches them: DART itself is deferred, see `boosting` |
