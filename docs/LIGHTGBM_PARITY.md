...
|---|---|---|
| `lossguide` | **LightGBM's** algorithm and per-parameter defaults: `learning_rate` 0.1, 31 leaves, its categorical rules. **This is the SHIPPED default policy**, `python/mojotrees/sklearn.py` line `grow_policy="lossguide"` and `tree.grow_tree`'s `grow_policy: Int = GROW_LEAFWISE` | **ours: `n_estimators` 100, the same as LightGBM.** Corrected 2026-08-17: this cell read "ours: `n_estimators` 72, where LightGBM ships 100", which was the other half of the same unimplemented 2026-08-16 decision the `symmetrictree` row already withdraws. At head `python/mojotrees/sklearn.py` ships `n_estimators=100` and `boosting.BoosterParams.default()` returns `(100, 0.1, TreeParams.default())`, so the budget is not carved out of the LightGBM mirror at all under this policy. The 72 is a BENCH ARM, `bench/real_data/frontier.py`'s `SHIPPED_N_ESTIMATORS`, and not a shipped default |
| `symmetrictree` (**OPT IN, not the default**, corrected 2026-08-17: this cell read "the default policy" and that was never true of the code. `python/mojotrees/sklearn.py` ships `grow_policy="lossguide"` and `n_estimators=100`, and git shows `grow_policy="symmetrictree"` has never been the estimator's default value while `n_estimators=360` has never existed at all. A 2026-08-16 decision to make symmetric the shipped default at 360 trees was recorded and never implemented, and this cell described the decision as though it had shipped. Selecting it DOES flip the whole default set to CatBoost's, because `python/mojotrees/sklearn.py` resolves `catboost_mode = grow_policy == "symmetrictree"`, so everything to the right of this cell is accurate for a fit that asks for it) | **CatBoost's** algorithm and SOME of its per-parameter defaults. **CORRECTED 2026-08-18, and this cell has now overclaimed twice.** What the mode ACTUALLY supplies, read from `python/mojotrees/sklearn.py` and `params.mojo::_apply_catboost_mode_defaults` rather than from intent: symmetric growth, `max_depth` 6, `learning_rate` 0.03 with CatBoost's rate derivation left enabled, `l2_leaf_reg` 3.0, `ctr` on, `random_strength` 1.0, and CatBoost's per-objective `leaf_estimation_iterations` and `boost_from_average`. **THREE THINGS THIS CELL CLAIMED AND THE CODE DOES NOT DO.** `bootstrap_type` is unset, so `_resolve_bootstrap` takes `kind = "no"` and there is NO MVS and no subsampling; the mode has no bootstrap branch at all. `score_function` is unset, so it resolves to `"l2"`, not Cosine, and that pairing is the worst of the three available, because `random_strength=1.0` is calibrated against Cosine and carries the wrong noise scale under L2. And `max_bin` stays 255 with no mode branch, so `border_count` 254 is not set either. Also unchanged by the mode: `min_data_in_leaf` stays 20 where CatBoost's is 1, and `max_cat_to_onehot` stays 4 where CatBoost's `one_hot_max_size` is 2. **Anyone configuring a CatBoost-paired benchmark arm from this cell before today would have believed they were running MVS, Cosine and 254 borders and would have been running none of them** | **ours: `n_estimators` 100**, where CatBoost ships 1000. This column said 360 until 2026-08-17, which contradicted the correction in this same row: 360 is the tree count of an unimplemented decision, the mode does not change the tree count at all, and 100 is LightGBM's `num_iterations` gated by `tools/check_parity.py`. Selecting CatBoost mode therefore gives you CatBoost's depth, rate, bootstrap, scoring and `l2_leaf_reg` at a TENTH of CatBoost's tree budget, which is the largest unmatched thing about the comparison and is not a defect in the mode, it is the shipped tree count |
...
**Corrected 2026-08-17, and the correction is that neither count is a shipped
default.** The paragraph above is still right about the principle and was wrong
about where those two numbers live. `python/mojotrees/sklearn.py` ships
`n_estimators=100` on both policies and `grow_policy="lossguide"`;
`boosting.BoosterParams.default()` returns 100 as well. 72 and 360 come from
`bench/real_data/frontier.py`'s `SHIPPED_N_ESTIMATORS`, which is the harness's
record of a 2026-08-16 decision that was never implemented in the library. So a
reader who finds either count has found a BENCH ARM. `docs/design/ACCURACY_GAP.md`
carries the full trace and is authoritative on it.
...
lossguide at 72, exist to show **what the harness records as our defaults** and
are **never** a claim of a win against a competitor at a different count. (That
clause read "to show **what we ship**" until 2026-08-17. It is corrected because
the library ships 100 on both policies; those two rows carry the harness's
`SHIPPED_N_ESTIMATORS`, which encodes a decision the code never took.) The report enforces that
...
| GPU split selection on the device | supported | yes | yes | yes | yes | yes | yes | yes | `src/mojotrees/gpu_split_search.mojo`, reached from `grow_tree_gpu` and from `MOJOTREES_GPU_SPLIT_STRATEGY=device`. **On by default for every eligible shape since 2026-08-16.** It was off below a measured crossover; the sweep that crossover was owed found none at any shape from 5.0M to 70.0M normalized work, with the device search winning everywhere and winning by more at the smaller shapes, so both thresholds were withdrawn. `SPLIT_SEARCH_AUTO` now resolves to the host scan only on eligibility grounds -- a configuration the device kernel cannot express, a resident frontier that does not fit, or unmeasured hardware. It grows over a device-resident frontier, so a split builds one histogram and subtracts for the sibling, as the host-search grower does; see the measurements under "GPU split selection" below |
...
| `class_weight` | different | `MojoTreesClassifier(class_weight=...)`: `"balanced"` or a `{label: weight}` dict, folded into the row weights before training, so there is one weighting mechanism rather than two. The estimator uses scikit-learn's rule for `balanced` (row counts, not weighted counts); `class_weight.balanced_class_weights` in Mojo counts weighted rows when it is handed a `sample_weight`, so the two surfaces agree only on an unweighted fit. Folding into the row weights is a divergence and not only an implementation choice: the folded weight reaches `boosting._base_score` and every eval metric, where LightGBM's class weight reaches neither. The `is_unbalance` / `scale_pos_weight` row in section 7 states the difference and carries the LightGBM line numbers. `src/mojotrees/class_weight.mojo` |
...
| `lambda_l2` | supported | The parameter is LightGBM's; the **default is a declared divergence, 1.0 here where LightGBM ships 0.0**. `TreeParams.default()` returns `TreeParams(31, 20, 1.0, 1e-3, 0.0)` and `_LAMBDA_L2` in `python/mojotrees/sklearn.py` is 1.0, which is what a Python fit resolves an unset `lambda_l2` from. The reason is recorded damage at 0.0 on three scenarios (`docs/design/ACCURACY_GAP.md` section 3.4: -1.6 percent of excess error on dense regression, 0.75x average precision on `imbalanced_binary`, 3.31x worse multiclass logloss), and what the departure spends is the standing rule that `lossguide` mirrors LightGBM. `tools/check_parity.py` asserts every stock default across all three literals and carries this one in `STOCK_DIVERGENCES` with its reason and its exit condition. Fits that do not set it explicitly are affected: `lambda_l2` is the denominator of the Newton step |
...
| Categorical bin count | **different** | **LightGBM has no categorical bin limit and mojotrees caps at 254.** `src/io/bin.cpp:461-462` loops `while (cur_cat_idx < distinct_values_int.size() && (used_cnt < cut_cnt || num_bin_ < max_bin))`, a **disjunction**, with `cut_cnt = RoundInt((total_sample_cnt - na_cnt) * 0.99f)` at `:443-444`. So the stop condition is **99 percent row coverage** and `max_bin` is a **lower** bound, not a cap; LightGBM's own loader says so at `dataset_loader.cpp:1581-1584`, that `max_bin` "may be ignored with a large number of categories". Its storage widens automatically, uint8 then uint16 then uint32 (`bin.cpp:618-628`). **MEASURED 2026-08-17, replacing an estimate that was wrong in three ways.** Every record in `bench/real_data` carries per-feature bin counts in `model.bins.counts`, so this never needed estimating; what stood here said "roughly 991 and 19,801 bins", naming two columns and guessing both counts. Read off `bench/real_data/results/20260817T102725Z-cat1m/records.json` on the five categorical columns at cardinalities 8, 64, 1000, 20,000 and 200,000: **mojotrees runs 9, 65, 255, 255, 255 and LightGBM runs 9, 65, 989, 19,433, 15,952.** So **three** of our columns are truncated rather than two, and the 200,000-level column gets 15,952 bins from LightGBM. In row-coverage terms we resolve 26.5, 1.8 and 10.8 percent of rows on the three wide columns where LightGBM resolves 98.9, 98.1 and 44.5 percent. **That the difference is the entire measured `average_precision` gap is now computed rather than asserted**: a binning oracle over this scenario's per-level effects scores 0.552770 average precision at LightGBM's bin counts against 0.442548 at ours, a 19.9 percent loss where the measured gap is 12.02 percent, so the ceiling more than fully accounts for the failure. Working in `docs/design/ACCURACY_GAP.md` section 7. **The same measurement retires the `UInt16` objection at the end of this row**: LightGBM does not exceed `UInt16` on this scenario either, so matching it does not require a wider integer than 65,535, and a named refusal above that is not the blocker it reads as. **And it bounds the CTR remedy below**: CatBoost runs CTRs on this scenario and scores 0.440563, 8.1 percent behind LightGBM and barely above our own 254-bin oracle, so a CTR default is unlikely to clear the 10 percent gate on its own. **A remedy is PROPOSED and its headline figure is WITHDRAWN as unearned, 2026-08-17.** What stood here claimed that `ctr="auto"`, which sources a target statistic from the raw category codes and therefore has no bin ceiling, takes average precision from 12.02 percent worse than LightGBM to **3.63 percent better** while still training 1.35x faster, and it cited `bench/results/COMPARISON_RUN_2026-08-16.md` plus "the CTR lane's rerun". **Neither number is in either place.** That artifact carries exactly one average precision for this scenario, 0.421925 against LightGBM's 0.479591, and it carries it as a FAIL (`COMPARISON_RUN_2026-08-16.md:198` and `:206`). No records file under `bench/real_data/results/` contains a `high_cardinality_categorical` cell with `ctr` set to anything but `off`, and none reports an average precision other than 0.421925; the 2026-08-17 rerun reproduced 0.421925 to the byte, which is what a scenario whose categorical handling never changed should do. So "the CTR lane's rerun" is not a locatable record and the two figures have no provenance. The mechanism is still the most promising candidate, for the structural reason that a target statistic reads the raw code and so cannot be truncated by a bin ceiling, and that reason is worth acting on. It is a HYPOTHESIS until a recorded arm carries it, and the measurement that would settle it is one `high_cardinality_categorical` cell at `ctr="auto"` beside the existing `ctr="off"` cell in one window. Until then no number here may be quoted. It is **opt-in under lossguide** because a CTR is CatBoost's mechanism rather than a LightGBM mirror, and this file's rule is that lossguide mirrors LightGBM; under CatBoost mode it is **`"on"`, the mirror, since 2026-08-17** (see below). **`"auto"` and `"on"` are two different rules and this row uses both words for a reason.** `"auto"` gives CTR columns to the categorical columns that overflowed their category table; `"on"` is CatBoost's own rule, `uniqueValues > one_hot_max_size` at 2, which replaces nearly every categorical column. The measurement quoted here is `"auto"`, and it is a fair measurement of `"on"` on that scenario only because a 200,000-level column overflows under either rule. On a NARROW categorical column the two diverge completely, and that divergence is what made CatBoost mode's default `"auto"` a defect rather than a preference: a six-level column is not replaced, stays inside `BinnedMatrix.usable`, is still offered to the split search, and the symmetric grower then refuses the fit. CatBoost mode's default was moved to `"on"` on 2026-08-17, moved back the same day, and moved to `"on"` again later that day once the two things that broke it were BUILT rather than argued about. What broke it, and what closed it (catalog A40): `SimpleCtrConfig.catboost_defaults()` was **not inert on a dataset with no categorical column** -- it demanded CTR target borders, whose selection was implemented only for a two-valued target, so `MojoTreesRegressor(grow_policy='symmetrictree')` on an ordinary continuous target **raised**, an opted-in policy on the commonest task. (This clause read "the shipped default policy on the commonest task" and is corrected 2026-08-17, for the same reason the `symmetrictree` row above is: the shipped default policy is `lossguide` and `symmetrictree` has never been the estimator's default. The defect was just as real, since a user who types that one argument and nothing else could not fit; it was not reachable without typing it.) `trainset._build_ctr` now decides its source columns from the FEATURE side before it asks for a target border, so a bundle with nothing to replace reads the target not at all; and `ctr_columns.select_target_borders` is CatBoost's own rule, `BuildTargetClassifier` -> `SelectBorders` -> MinEntropy at one border over the actual target (`target_classifier.cpp:18-37`, `cat_feature_options.cpp:162`, v1.2.10), so a continuous target needs no special case. `ctr_target_border_count` and `ctr_target_border_type` are estimator parameters now. **So CatBoost mode mirrors CatBoost's categorical handling.** The bar the default is held to is `python/tests/test_catboost_mode_categorical.py`, which is four positive fits and not one: plain symmetric regression, symmetric regression with `ctr='on'` and numeric columns only, the same beside a categorical column, and classification with a categorical column and `random_strength=1.0`. **Matching LightGBM's rule itself is held**, and the cost is why: the 256-bit set is baked into four-word loops in `src/mojotrees/split.mojo` (three sites), `src/mojotrees/tree.mojo`, `src/mojotrees/gpu_tree_tables.mojo`, `src/mojotrees/gpu_categorical.mojo`, `src/mojotrees/gpu_predict.mojo` and `src/mojotrees/gpu_fused_round.mojo`; a **kernel ABI** passing the set as four scalar `UInt64` arguments; the **distributed wire record** (`CANDIDATE_WORDS = 8 + 2 * CAT_BITSET_WORDS`); six alignment checks across `src/mojotrees/serialize.mojo` and `src/mojotrees/model_editing.mojo`; and a Python mirror in `python/mojotrees/inspection.py`. `MAX_BINS = 256` separately gates 20 load sites in `src/mojotrees/histogram.mojo`, six in `src/mojotrees/tree.mojo`, and 256-wide shared-memory planes across eight GPU files. **And it still would not reach the column that motivated it**: 200,000 levels exceeds `UInt16`, so a wider integer needs a named refusal above 65,535 anyway, where a CTR has no ceiling at all |
...
| `is_unbalance` / `scale_pos_weight` | **different** | Both are in `src/mojotrees/class_weight.mojo` under their LightGBM names (`unbalanced_sample_weight`, `scale_pos_weight_rows`), and neither is a constructor parameter on the Python classifier. **The estimator's `class_weight` is not equivalent to either one.** LightGBM (`src/objective/binary_objective.hpp`, 4.7.0) puts both knobs into `label_weights_` (`:89-103`, where `is_unbalance` lifts whichever class is the minority and `scale_pos_weight` then multiplies the positive entry) and multiplies that weight into the gradient and the hessian and into nothing else (`:114-134`); `BoostFromScore` (`:139-165`) reads `metadata.weights()` and never `label_weights_`, so its init score is the unweighted base rate and its metrics move no early-stopping decision. mojotrees expands every class weighting into an ordinary per-row `sample_weight` before any trainer is called, so the same weight also reaches `boosting._base_score` (weighted mean, then logit) and every eval metric, and early stopping stops at a different round. Same two numbers in, two different models out; the gradient scaling is the only part the two engines share. **The design is kept.** One weighting mechanism rather than two is what makes a class-weighted fit bit-identical to the same fit with those row weights passed by hand, and a model told that positives count four times should start from the reweighted base rate and be early-stopped on the reweighted loss. What is contractual is that this is a difference: a differential against LightGBM on an imbalanced binary fit must not expect the two to agree, and a ported script that sets `scale_pos_weight` sees a different init score and a different early-stopping trace here even where the gradient scaling matches. It is not a constant factor apart either. A uniform rescale of the row weights is not gain-invariant while `lambda_l2` is 1.0 and `min_sum_hessian_in_leaf` is 1e-3, since `G^2 / (H + lambda_l2)` and `-G / (H + lambda_l2)` both move when `G` and `H` scale and `lambda_l2` does not, so the two pick different splits and write different leaf values. And on a majority-positive label LightGBM lifts the minority while `class_weight.unbalance_scale` returns `negatives / positives` and always leaves the negatives at 1.0, so our multiplier shrinks the positives where LightGBM lifts the negatives; the ratio between the classes is the same in both, which is exactly why a constant factor in the gradient is not a constant factor in the model. `unbalance_scale` also derives that ratio from weighted counts where LightGBM's `Init` counts rows, so the two coincide only on an unweighted fit |
...
| GPU split selection | supported | **On by default for every eligible shape since 2026-08-16.** It searches each node's histogram on the device and downloads one 136-byte record instead of the histogram. It keeps a histogram slot per live leaf, so a split builds only its smaller child and derives the sibling with a device subtraction, and searches both children in one launch pair: one histogram build, one wait, and 272 bytes per split. It was off below `normalized_split_work >= 50,000,000` (12,500,000 for depth-wise), on the reading that Float32 device gains can flip near-tie decisions and that a few percent does not pay for that. BOTH HALVES OF THAT WERE MEASURED ON 2026-08-16 AND BOTH WERE WRONG. Speed: four shapes, the two arms interleaved in one process, five repeats each, box verified quiet at every shape boundary -- 100,000 x 50 (5.0M work) 1.695 s host against 0.918 s device, 250,000 x 100 (25.0M) 2.494 against 1.862, 463,715 x 90 (41.7M) 3.041 against 2.309, 700,000 x 100 (70.0M) 4.041 against 3.128. Every shape resolved with disjoint ranges, and the device margin is LARGEST at the smallest shape (1.85x falling to 1.29x), which is the signature of a per-node cost and the opposite of what a profitability gate assumes. Fidelity: at 463,715 x 90 the two arms' predictions were bit-identical to each other (max absolute difference 0.0000 over 51,630 test rows) and both diverged from the CPU by exactly the same amount, so the host scan did not deliver CPU-identical splits at that shape either. Both thresholds are withdrawn and neither is replaced; the host scan remains for ELIGIBILITY refusals (an inexpressible configuration, a resident frontier that does not fit, unmeasured hardware), which are not size questions. The earlier figures this row carried (24% behind at 50000 x 100, indistinguishable at 250000 x 100) were single or triple runs in separate thermal windows and are superseded by the interleaved sweep. Scope: this compares two GPU arms and says nothing about the CPU/GPU crossover. `MOJOTREES_GPU_SPLIT_RESIDENT=0` forces the older loop, which builds both children and waits twice per split. `src/mojotrees/gpu_split_search.mojo` |
