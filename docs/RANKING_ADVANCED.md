...
| The generalized pair loop reduces to `ranking._fill_query_lambdas` | **Verified, in-tree.** This row used to read "**Unverified**, and the check that would verify it is listed UNRUN". `test_default_is_not_advanced_and_matches_train_ranker` is that check and it has run: on the default `AdvancedRankParams`, `train_ranker_advanced` and `train_ranker` produce the same tree count and bit-identical `predict_row` over every row. The handoff file it cited no longer exists (`ls handoffs/` lists five files and that is not one of them), so the UNRUN pointer was dangling as well as stale |
...
ready-to-apply patch in a handoff, and the whole `handoffs/` directory has
since been deleted, so the pointer is dropped rather than repeated.
