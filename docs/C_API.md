...
- `capi/mojotrees.h` is the contract, and documents every function.
...
each call actually reaches inside mojotrees.
...
|---|---|
| `mojotrees_train_dense` | `fit` / `fit_multiclass` (`model.mojo`) |
| `mojotrees_predict`, `mojotrees_predict_raw`, `mojotrees_predict_ex` | `Model.predict_batch` / `MulticlassModel.predict_batch` (`model.mojo`) |
| `mojotrees_save_model`, `mojotrees_load_model` | `serialize.mojo` |
| `mojotrees_model_dump_json` | `dump_model` / `dump_multiclass_model` (`inspection.mojo`) |
| `mojotrees_feature_importance` | `split_importance` / `gain_importance` (`importance.mojo`) |
| `mojotrees_parameter_keys` | `SUPPORTED_KEYS` (`params.mojo`) |
| the `device` argument | `resolve_device` (`device.mojo`, over `device_policy.mojo`) |
| `parameters` | `parse_params` (`params.mojo`) |
| `mojotrees_gpu_available` | `gpu_available` (`device.mojo`) |
...
where anything runs. A change to how mojotrees does any of that reaches C
...
`MOJOTREES_ABI_VERSION` is incremented whenever the header gains
...
| 2 | `mojotrees_predict_ex`, `mojotrees_model_num_iterations`, `mojotrees_gpu_available`, `mojotrees_model_dump_json`, `mojotrees_string_free`, `MOJOTREES_DEVICE_*`, `MOJOTREES_PREDICT_*` |
| 3 | `mojotrees_feature_importance`, `mojotrees_parameter_keys`, `MOJOTREES_IMPORTANCE_*` |

A caller built against version N works unchanged against any library
reporting at least N. Test accordingly:

```c
if (mojotrees_abi_version() < 2) { /* no predict_ex here */ }
```
...
The library version, from `mojotrees_library_version`, moves independently
...
- The **model file format**, which `serialize.mojo` versions itself, and
  which `mojotrees_load_model` reads. A file written by one release loads in
  another under the compatibility policy in
  [COMPATIBILITY_POLICY.md](COMPATIBILITY_POLICY.md), not under this
  version.
- The **inspection schema** that `mojotrees_model_dump_json` returns, which
...
mojotrees struct layout is exposed, so a release can change any internal
...
- Model handles are opaque. The accessors report what a caller needs to size
  a buffer, and `mojotrees_model_dump_json` covers everything else. There is
...
| `mojotrees_error_message` result | the error object | nothing |
| `mojotrees_model_dump_json` result | caller | `mojotrees_string_free` |
| `mojotrees_parameter_keys` result | caller | `mojotrees_string_free` |
...
the caller's allocator, which is why `mojotrees_string_free` exists at all.
...
writes the reason into the `MojoTreesError *` passed to it. A `NULL` error
object discards the message and keeps the status code.

| Code | Meaning |
|---|---|
| `MOJOTREES_ERROR_INVALID_ARGUMENT` | a `NULL`, a nonpositive dimension, a buffer too small, a shape that does not match the model, an undefined flag or device constant, or a parameter string that is wrong |
| `MOJOTREES_ERROR_TRAINING` | well-formed arguments, but training failed — poisson with a negative label, for instance |
| `MOJOTREES_ERROR_IO` | a model file could not be read or written |
| `MOJOTREES_ERROR_UNSUPPORTED` | a real mojotrees feature asked for the wrong way: a parameter string naming something only the Mojo API exposes, or a device request the policy refused |
...
`MOJOTREES_DEVICE_CPU`, `_GPU`, and `_AUTO` are the same three values the
...
- **CPU** is the dependable path: Float64 throughout, every objective, every
  entry point.
- **GPU** fails with `MOJOTREES_ERROR_UNSUPPORTED` and the policy's own
...
`mojotrees_gpu_available()` answers whether a GPU request is worth making at
...
argument to `mojotrees_predict_ex`, because a model is trained once and
scored in many places.

`mojotrees_predict` and `mojotrees_predict_raw` are fixed to the CPU and to
...
`mojotrees_predict_ex` takes LightGBM's `(start_iteration, num_iteration)`
...
`num_class`; `mojotrees_model_num_iterations` reports the unit ranges are
expressed in, and `mojotrees_model_num_trees` reports the other one.
...
`mojotrees_model_num_classes(model)`, which is 1 for every single-output
...
concurrently. A single `MojoTreesError`, or a single model being freed, must
...
- Load the library dynamically and gate on `mojotrees_abi_version()` if you
  want one binding to work across mojotrees releases. Everything in this ABI
  is reachable by symbol name; nothing requires a compiled-in struct.
- Size every prediction buffer from `mojotrees_model_num_classes`, not from
...
- Do not parse error messages. They are for humans; branch on the status
  code.
- `r/mojotrees` is the first in-tree consumer of this ABI and the worked
  example of the paragraphs above: `r/mojotrees/src/mojotrees_r.c` sizes its
  ...
  jump. `r/mojotrees/configure` shows how to fail at configure time on an
  ABI older than a binding needs.
- Keep a binding's parameter list from drifting by reading
  `mojotrees_parameter_keys` rather than hardcoding one. The list is primary
