...
| `gpu_vendor_policy` | EXPERIMENTAL | consolidation_K2 | CUDA and HIP occupancy policy, merged from the gpu_cuda_policy / gpu_amd_policy twins (f23bd1b). Reached only from its test until a discrete-GPU trainer consults it; that is the same status the twins had. handoffs/migration_20_device_policy.md (deleted, recover with git log --all --diff-filter=D -- handoffs/migration_20_device_policy.md) |
...
this audit was non-blocking pending exactly this catch-up. It is blocking now
(`--strict`, `.github/workflows/ci.yml`), so the next lane that lands a module
no entry point reaches has to classify it in the same change rather than leave
it for whoever next reads the tool's output. Being unreached is a legitimate
state and none of these rows is an accusation; going unrecorded is not.
...
| `gpu_split_search` | `train_gpu` | **the device scan, for every eligible shape**, since the size threshold was withdrawn on 2026-08-16 for having no measured crossover behind it; the host scan remains for eligibility refusals only | `MOJOTREES_GPU_SPLIT_STRATEGY=host` forces the host scan |
...
| `device_policy` crossover table | `device`, and through it every `fit` | one rule, scoped to Metal on an M4 for squared error, single output, dense, 50 or more features, and `AUTO_GPU_MIN_ROWS` = 250,000 rows and above. Reachable since 2026-08-16: `DeviceCapabilities.detect()` names the hardware from the build target, `parse_apple_generation` parses the architecture string a device really reports, and the six trainer entry points pass the objective they hold. `auto` selects the GPU there and the CPU everywhere else, with a warning that says which half of "does not cover" applied. `resolve_device`'s four-argument form declares no objective and so still resolves to the CPU at every shape. `MOJOTREES_DERIVATIVE_PRECISION=float64` blocks the GPU outright (`BLOCK_DERIVATIVE_PRECISION`), so `auto` routes to the CPU at every shape and an explicit `gpu` raises | pass `objective` to `resolve_device`, or call `resolve_device_full` / `decide_device_report`; `MOJOTREES_AUTO_MIN_CELLS` for hardware no rule covers; `decide_device_report_reported` for a caller holding an open `DeviceContext` |
