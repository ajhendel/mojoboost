# Building mojotrees: gradient-boosted trees for the GPU inside a Mac
...
The premise behind mojotrees is that the accelerator already present in every
...
The original GPU histogram path in mojotrees exposed the central problem: if
...
mojotrees follows one architectural rule: Mojo decides; Python asks and
formats.
...
page migration, synchronization, mapping, and contention costs. mojotrees's
...
model = MojoTreesClassifier(device="auto")
model.fit(
    X_train,
    y_train,
...
I have built mojotrees with extensive assistance from coding agents. They make
...
- a clean `pip install mojotrees` on a supported Mac;
...
If that result is strong, mojotrees can offer something concrete: native
