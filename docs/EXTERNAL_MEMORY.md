...
| Multiclass | Yes | `train_external_multiclass`, `train_external_sparse_multiclass`. CPU only, and the reason given here used to be "as `trainset.train_dataset_multiclass` is", which is no longer true: that function was fixed to dispatch on the device it resolves. `train_external_multiclass` was not, and it still calls `resolve_device` and discards the answer (`_ = resolve_device(...)` at `src/mojotrees/external_memory.mojo:2001`) before calling the CPU trainer unconditionally. So a `device=GPU_DEVICE` here trains on the CPU and reports nothing about it, which is the same dispatch bug that made every earlier multiclass GPU benchmark a mislabelled CPU fit. Treat this row as a known defect rather than as a design |
