...
| A4 | Bayesian bootstrap / `bootstrap_type` (**verified from source**, see A4 note) | Row weighting instead of row dropping: every row kept, each given weight `(-log(U + 1e-100)) ** bagging_temperature`, redrawn once per tree | Only when on | Yes; built in `sampling.mojo`, off by default | CPU (`sampling.mojo`) | (3) when on; **carries a constant-hessian exclusion**, see the note |
...
| A6 | `leaf_estimation_iterations` (Newton steps per leaf after the structure is fixed) | Re-estimate leaf values 1..k times | Only when > 1 | Cheap opt-in later | CPU | later |
