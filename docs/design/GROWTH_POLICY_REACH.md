...
| `MOJOTREES_GPU_OBLIVIOUS_SUBTRACT` (sibling subtraction) | `gpu_leaf_batching.mojo` | REACHES, unconditionally and with no switch (`enqueue_desc_child` folds it in) | REACHES, unconditionally (`enqueue_resident_leaf_subtracting`) | REACHES behind the switch, because this plane is the only one that ever dropped it |
