...
def f64_buffer(addr: Int, n: Int) raises -> List[Float64]:
    ...
    The one home for this read; `_mojotrees.mojo` and the capability
    modules all take it from here. One bulk copy, not an element-by-element
    append: the caller's buffer and the list hold the same bytes in the
    same order, so there is nothing per element to decide,
    `unsafe_uninit_length` skips the zero fill that `resize` would do, and
    the copy that follows writes every one of those bytes. Measured over 25
    million elements, best of three alternating runs in one process:
    0.0061 s appending, 0.0043 s copying.
...
def int_buffer(addr: Int, n: Int) raises -> List[Int]:
    ...
    list. Same bulk copy as `f64_buffer`, for the same reason: int64 in,
    Int out, identical bytes."""
    if addr == 0:
        raise Error("null buffer address")
    if n < 0:
        raise Error("buffer length must not be negative, got ", n)
